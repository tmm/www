...
With that in mind, I did a bunch of research. I downloaded and tested an absurd number of editors. I didn't use any metrics to compare them other than how it *felt* to write. Often, how it felt depended on how an editor treated Markdown.
...
We'll use the part of the [Markdown Wikipedia article](https://en.wikipedia.org/wiki/Markdown) as a test example.
...
The challenge with block preview editors, like Roam, is the scoping of previews is too broad. In the example paragraph, there are multiple [inline](https://daringfireball.net/projects/markdown/syntax#span) elements that are independent from each other, yet all of them depend on the containing paragraph's state for triggering previews.
...
While the concept of reading in rich text and writing in plain text solves challenges faced by other Markdown editors, it's not perfect. I haven't figured out how to handle empty lines. Empty lines make sense when typing in plain text (just press <kbd>Enter</kbd>), but don't translate well to HTML previews since Markdown collapses [whitespace](https://en.wikipedia.org/wiki/Whitespace_character) during compilation.
